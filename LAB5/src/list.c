...
unsigned long simulate_service(struct mem_slot **mem, int *mem_used, int num)
{
    struct queue queue;
    ...
    while (machine.processed_count < num)
    {
    ...
    {
        model = (double)(PROCESSING_END + PROCESSING_START) / 2 * num * (1 / 0.2);

    }
    else
        model = ((double)(COMING_END + COMING_START) / 2) * num;        
...
void queue_compare(struct mem_slot **mem, int num)
{
    int mem1 = 0, mem2 = 0;
    unsigned long t1 = simulate_service(mem, &mem1, num);
    unsigned long t2 = simulate_service_arr(&mem2, num);
...
{
    queue_compare(mem, 100);
    queue_compare(mem, 500);
    queue_compare(mem, 1000);
    queue_compare(mem, 2000); 
}
